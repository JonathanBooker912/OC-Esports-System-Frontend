...
const height = computed(()=>{
  const length = eventData.value.endTime.diff(eventData.value.startTime, 'minutes') / 60
  return `height: ${Math.floor(length * 52)}px;`
})

const topOffset = computed(() => {
  return `margin-top: ${(eventData.value.startTime.minutes() / 15) * (52/4)}px;`
})
...
<template>
    <v-card v-if="loaded" class="w-100 pa-1" :style="height + topOffset" :color="eventData.team.teamColor" @click="emitShow">
