...
const getUnsignedFormsForVersion = async () => {
    formVersions.value.forEach(async (form) => {
        await FormSignatureServices.getDirectorUnsigned(form.id)
        .then((response) => {
...
const getFormVersions = async () => {
    await FormServices.getFormVersions(props.form)
    .then((response) => {
    ...
    <v-expansion-panels v-if="!loading">
        ...
        >
            ...
            <v-expansion-panel-text>
                <UnsignedFormVersion :unsignedForms="version.unsignedForms"/>
            </v-expansion-panel-text>
